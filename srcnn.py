...
class SRCNN(nn.Module):
    def __init__(self):
        ...
        # padding formula ((w-f+2p)/s)+1 = output where input(w),output and stride(s) is 1 so p = (f-1)/2
        # where f is the kernel size.

        # conv1 layer padding => (9-1)/2 = 4
...
# convert SR_image from tensor to numpy array and reshape to match HR image shape
# print("HR_image shape:", HR_image.shape)
...
# Compute PSNR for LR and SR images
...
# Print PSNR values for LR and SR images
print("The psnr for LR image is: ", LR_psnr)
print("The psnr for SR image is: ", SR_psnr)
